...
:: Set the project root relative to this batch file
set "ROOT=%~dp0.."

:: ─── 1. API Server (FastAPI) on port 8000 ───
echo  [1/4] Starting API Server on port 8000...
start "Proctor360 API" cmd /k "cd /d %ROOT%\services\api && uvicorn app.main:app --host 0.0.0.0 --port 8000 --reload"
timeout /t 2 /nobreak >nul

:: ─── 2. AI Engine on port 8100 ───
echo  [2/4] Starting AI Engine on port 8100...
start "Proctor360 AI Engine" cmd /k "cd /d %ROOT%\services\ai-engine && uvicorn app.main:app --host 0.0.0.0 --port 8100 --reload"
timeout /t 2 /nobreak >nul
